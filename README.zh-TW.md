...
- **多活動分區** — 同一套系統管理多個活動（Eurobot 2025 / 2026 / …），可即時切換使用中；活動可選擇性指定成員以限制登入與精簡負責人下拉選單
- **完整備份** — 資料庫與照片打包成單一 ZIP，可一鍵還原
- **多語介面** — 中英切換、Light / Dark / System 三種主題，預設深色模式
...
1. **列印貼紙** — 主應用 → 貼紙列印 → 選尺寸和物品 / 箱子 → 產生 PDF（單一箱子也可從箱子詳情頁直接下載）
...
每個活動可以透過 **管理後台 → 活動管理 → 成員** 限制成員清單。一旦設定至少一位成員：

- 主應用的負責人下拉選單只會顯示該活動的成員（人多時更好找）
- 非成員用 Slack 登入會被拒絕，登入頁顯示明確錯誤訊息
- 已登入的非成員下次操作時 session 會被自動清除

**沒設成員（清單為空）的活動視為不限制**（所有使用者皆可選與登入）。
